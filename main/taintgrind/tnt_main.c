...
// For translation table 32 bit taint -> 8 bit SecMap entry
#define TRANSL_TABLE_SIZE	  256
...
static TranslTable translTable;
...
/* --------------- lookup vbits in translTable --------- */

UWord get_vbits_from_transltable( UWord secmapentry ) {
	if (secmapentry == VA_BITS8_UNTAINTED) {
		//VG_(printf)("get_vbits_from_transltable -> VA_BITS8_UNTAINTED\n");
		return V_BITS16_UNTAINTED;
	}

	if (secmapentry > TRANSL_TABLE_SIZE)
		VG_(printf)("too big: secmapentry=%ld\n", secmapentry);
	tl_assert(secmapentry <= TRANSL_TABLE_SIZE);
	UWord result = translTable.entry[secmapentry];
#ifdef VERBOSE_DEBUG
	VG_(printf)("lookup in translTable: translTable[%l] = %lx \n", secmapentry, (UInt)result);
#endif
	return result;
}

static INLINE
UWord get_transltable_entry( UWord vabits16 ) {
	// translate vabits16 with translation table

	if (vabits16 == VA_BITS16_UNTAINTED || vabits16 == VA_BITS8_UNTAINTED)
		return VA_BITS8_UNTAINTED;

	if (vabits16 == V_BITS16_UNTAINTED) {
		//VG_(printf)("get_transltable_entry: vabits16=VA_BITS16_UNTAINTED=%x\n", VA_BITS16_UNTAINTED);
		return VA_BITS8_UNTAINTED;
	}

	UInt i=1;
	// search for first unused entry
	while (translTable.entry[i] != vabits16 && translTable.entry[i] != 0) i++;
	translTable.entry[i] = vabits16;
#ifdef VERBOSE_DEBUG
	VG_(printf)("get_transltable_entry: translTable.entry[%d]=%lx\n", i, vabits16);
#endif
	vabits16 = i;
	tl_assert(i <= TRANSL_TABLE_SIZE);
	return i;
...
UWord TNT_(get_taint) ( Addr a ) {
	return get_vbits_from_transltable(get_vabits8_for_aligned_word32(a));
}
...
{
   /*VG_(printf)("tnt_LOADVn_slow (Addr=%p, nBits=%d, bigendian=%d)\n",
		   a, nBits, bigendian);*/
...
void tnt_STOREVn_slow ( Addr a, SizeT nBits, ULong vbytes, Bool bigendian )
{
   ...
   SecMap* sm      = get_secmap_for_writing(a);
   //VG_(printf)("tnt_STOREVn_slow: setting %p->vabits8[%d] = %x\n", sm, sm_off, vbytes);

   sm->vabits8[sm_off] = vbytes;
                                      ...
                                      UWord dsm_num ) //1329
{
   ...
   /*VG_(printf)("set_address_range_perms(a=%p, lenT=%d, vabits16=%x, dsm_num=%d\n",
		   a, lenT, vabits16, dsm_num);*/

   PROF_EVENT(150, "set_address_range_perms");


   // translate vabits16 with translation table
   vabits16 = get_transltable_entry(vabits16);
   ...
   while (True) {
      ...
      sm_off = SM_OFF(a);
#ifdef VERBOSE_DEBUG
      VG_(printf)("sm_off16=%d for a=%x\n", sm_off16, (UInt)a);
      VG_(printf)("sm_off=%d for a=%x\n", sm_off, (UInt)a);
#endif
...
void TNT_(make_mem_tainted) ( Addr a, SizeT len, UWord taint )//1608
{
   PROF_EVENT(42, "TNT_(make_mem_undefined)");
//   DEBUG("TNT_(make_mem_undefined)(%p, %lu)\n", a, len);
   set_address_range_perms ( a, len, taint, SM_DIST_TAINTED );
}

void TNT_(make_mem_defined) ( Addr a, SizeT len )
{
   PROF_EVENT(42, "TNT_(make_mem_defined)");
   //VG_(printf)("TNT_(make_mem_defined)(a=%p, len=%d)\n", a, len);
...
//   DEBUG("TNT_(copy_address_range_state)\n");
   ...
   if (nooverlap && aligned) {
      ...
      while (len >= 4) {
	   ...
	   VG_(printf)("Warning: overlapping copy range permission not implemented yet\n");
	   tl_assert(src-dst >= 4 || dst-src >= 4);
	      i = 0;
	      while (len >= 4) {
	         vabits8 = get_vabits8_for_aligned_word32( src+i );
	         set_vabits8_for_aligned_word32( dst+i, vabits8 );
...
{
	tl_assert(0); // is this actually used on ARM?
...
ULong tnt_LOADV64 ( Addr a, Bool isBigEndian )
{
...
#else
   {
      ...
      sm_off16 = SM_OFF_16(a);
      vabits16 = sm->vabits8[sm_off16];

      return get_vbits_from_transltable(vabits16);
   }
#endif
}
...
void tnt_STOREV64 ( Addr a, ULong vbits64, Bool isBigEndian )
{
...
#else
   {
      ...
      if (vbits64 != 0)
    	  //VG_(printf)("tntStoreV64(Addr a=%p, vbits64=%x, isBigEndian=%d)\n", a, vbits64, isBigEndian);
      ...
      vbits64 = get_transltable_entry(vbits64);

      if (UNLIKELY(vabits16 == (UWord)vbits64)) return;

      if (LIKELY( !is_distinguished_sm(sm)))
      {
    	 ((UShort*)(sm->vabits8))[sm_off16] = (UShort)vbits64;
      } else {
...
UWord tnt_LOADV32 ( Addr a, Bool isBigEndian )
{
...
#else
   {
      ...
      return get_vbits_from_transltable(vabits8);
   }
#endif
}
...
void tnt_STOREV32 ( Addr a, UWord vbits32, Bool isBigEndian )
{
...
#else
   {
      ...
      if (vbits32 != 0)
    	  //VG_(printf)("->tntStoreV32(Addr a=%p, vbits32=%x, isBigEndian=%d)\n", a, vbits32, isBigEndian);
      ...
      vbits32 = get_transltable_entry(vbits32);
      if (vbits32 == vabits8) return;

      if (!is_distinguished_sm(sm)) {
    	  //VG_(printf)("tnt_STOREV32: setting %p->vabits8[%d] = %x\n", sm, sm_off, vbits32);
    	  sm->vabits8[sm_off] = vbits32;
      } else {
...
UWord tnt_LOADV16 ( Addr a, Bool isBigEndian )
{
...
#else
   {
	  if(!VG_IS_4_ALIGNED(a)) {
		  //VG_(printf)("Address %p not aligned", a);
		  a = VG_ROUNDDN(a, 4);
		  //VG_(printf)(" -> rounding down to %p\n", a);
	  }
      UWord   sm_off;
      UChar vabits8;
      SecMap* sm;
      ...
      UWord result = get_vbits_from_transltable(vabits8);
#ifdef VERBOSE_DEBUG
      if (vabits8 != 0 && vabits8 != VA_BITS8_UNTAINTED)
    	  VG_(printf)("tnt_LOADV16 Addr a:%p vabits8=%x -> %lx\n", (void*)a, vabits8, result);
#endif
      return result;
   }
#endif
...
void tnt_STOREV16 ( Addr a, UWord vbits16, Bool isBigEndian )
{
...
#else
   {
      ...
      SecMap* sm;
      if (vbits16 != 0)
    	  //VG_(printf)("tntStoreV16(Addr a=%p, vbits16=%x, isBigEndian=%d)\n", a, vbits16, isBigEndian);
      ...
      sm_off  = SM_OFF(a);
      vabits8 = sm->vabits8[sm_off];
      vbits16 = get_transltable_entry(vbits16);
      if (vbits16 == vabits8) return;

      if (LIKELY( !is_distinguished_sm(sm)))
      {
    	  /*VG_(printf)("tnt_STOREV16: setting %p->vabits8[%d] = %x\n",
    			  sm, sm_off, vbits16);*/
    	  sm->vabits8[sm_off] = vbits16;
      } else {
         /* Slow but general case. */
...
//      VG_(printf)("TNT_(helperc_STOREV8) 8 0x%08lx\n", a);
...
#else
   {
      ...
      sm_off  = SM_OFF(a);
      vabits8 = sm->vabits8[sm_off];
      vbits8 = get_transltable_entry(vbits8);
      if (vbits8 == vabits8) return;

      if (LIKELY( !is_distinguished_sm(sm) ))
      {
    	  //VG_(printf)("tnt_STOREV8: setting %p->vabits8[%d] = %x\n", sm, sm_off, vbits8);
    	  sm->vabits8[sm_off] = vbits8;
          return;
      } else {
         /* Slow but general case. */
...
static void tnt_pre_syscall(ThreadId tid, UInt syscallno,
                           UWord* args, UInt nArgs)
{
	switch (syscallno) {
	case __NR_ioctl: //54
		//VG_(printf)("%d syscall: ioctl(%d, %p, %p, %d, %d, %d, %d, %d)\n",
		//	tid, args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
		switch (args[1]) {
		case VKI_BINDER_WRITE_READ:
//#ifdef VERBOSE_DEBUG
			VG_(printf)("%d syscall: ioctl binder WRITE_READ\n", tid);
//#endif
           	struct vki_binder_write_read* bwr = (struct vki_binder_write_read*)args[2];
#ifdef VERBOSE_DEBUG
           	VG_(printf)("write_buffer=%lu, write_size=%ld, write_consumed=%ld, read_buffer=%lu, read_size=%ld, read_consumed=%ld\n",
			bwr->write_buffer, bwr->write_size, bwr->write_consumed, bwr->read_buffer, bwr->read_size, bwr->read_consumed);
#endif
			UInt size = bwr->read_size;
			UInt *writebuf = (UInt*) bwr->read_buffer;
			UInt *end = writebuf + (size / sizeof(UInt));
			VG_(printf)("write_size=%d, writebuf=%p, end=%p\n", size, writebuf, end);
			while (writebuf < end) {
				UInt cmd = *writebuf++;
				if (cmd>0) VG_(printf)("%d %x ", cmd, cmd);
				switch ((int)cmd) {
#ifdef VERBOSE_DEBUG
				case 0: break;
				case VKI_BINDER_BR_ERROR:
					VG_(printf)("Binder read: Error\n");
					break;
				case VKI_BINDER_BR_OK:
					VG_(printf)("Binder read: OK\n");
					break;
#endif
				case VKI_BINDER_BR_TRANSACTION:
#ifdef VERBOSE_DEBUG
					VG_(printf)("Binder read: Transaction\n");
#endif
					//tnt_ppBinderTransactionData((struct vki_binder_transaction_data*) writebuf);
				...
				case VKI_BINDER_BR_REPLY:
					...
					break;
#ifdef VERBOSE_DEBUG
				case VKI_BINDER_BR_ACQUIRE_RESULT:
					VG_(printf)("Binder read: Acquire Result\n");
					break;
				case VKI_BINDER_BR_DEAD_REPLY:
					VG_(printf)("Binder read: Dead Reply\n");
					break;
				case VKI_BINDER_BR_TRANSACTION_COMPLETE:
					VG_(printf)("Binder read: Transaction Complete\n");
					break;
				case VKI_BINDER_BR_INCREFS:
					VG_(printf)("Binder read: Increfs\n");
					break;
				case VKI_BINDER_BR_ACQUIRE:
					VG_(printf)("Binder read: Acquire\n");
					break;
				case VKI_BINDER_BR_RELEASE:
					VG_(printf)("Binder read: Release\n");
					break;
				case VKI_BINDER_BR_DECREFS:
					VG_(printf)("Binder read: Decrefs\n");
					break;
				case VKI_BINDER_BR_ATTEMPT_ACQUIRE:
					VG_(printf)("Binder read: Attempt Acquire\n");
					break;
				case VKI_BINDER_BR_NOOP:
					VG_(printf)("Binder read: Noop\n");
					break;
				case VKI_BINDER_BR_SPAWN_LOOPER:
					VG_(printf)("Binder read: Spawn Looper\n");
					break;
				case VKI_BINDER_BR_FINISHED:
					VG_(printf)("Binder read: Finished\n");
					break;
				case VKI_BINDER_BR_DEAD_BINDER:
					VG_(printf)("Binder read: Dead Binder\n");
					break;
				case VKI_BINDER_BR_CLEAR_DEATH_NOTIFICATION_DONE:
					VG_(printf)("Binder read: Clear Death Notification Done\n");
					break;
				case VKI_BINDER_BR_FAILED_REPLY:
					VG_(printf)("Binder read: Failed Reply\n");
					break;
				default:
					VG_(printf)("warning: unknown binder command!\n");
#endif
				}
			}
			size = bwr->write_size;
			...
			while (writebuf < end) {
				UInt cmd = *writebuf++;
#ifdef VERBOSE_DEBUG
				VG_(printf)("%d %x ", cmd, cmd);
#endif
				switch ((int)cmd) {
#ifdef VERBOSE_DEBUG
				case VKI_BINDER_BC_FREE_BUFFER:
					VG_(printf)("Binder Command Free Buffer\n");
					break;
#endif
				case VKI_BINDER_BC_TRANSACTION:
					VG_(printf)("Binder Command Transaction\n");
					//tnt_ppBinderTransactionData((struct vki_binder_transaction_data*) writebuf);
					writebuf += sizeof((struct vki_binder_transaction_data*) writebuf) / sizeof(UInt);
					break;
				case VKI_BINDER_BC_REPLY:
					VG_(printf)("Binder Command Reply\n");
					//tnt_addTaint((struct vki_binder_transaction_data*) writebuf);
					TNT_(getTaintForOutgoing)((struct vki_binder_transaction_data*) writebuf);
					//TNT_ppBinderTransactionData((struct vki_binder_transaction_data*) writebuf);
					writebuf += sizeof((struct vki_binder_transaction_data*) writebuf) / sizeof(UInt);
					break;
#ifdef VERBOSE_DEBUG
				case VKI_BINDER_BC_ACQUIRE_RESULT:
					VG_(printf)("Binder Command Acquire Result\n");
					break;
				case VKI_BINDER_BC_INCREFS:
					VG_(printf)("Binder Command Increfs\n");
					break;
				case VKI_BINDER_BC_ACQUIRE:
					VG_(printf)("Binder Command Acquire\n");
					break;
				case VKI_BINDER_BC_RELEASE:
					VG_(printf)("Binder Command Release\n");
					break;
				case VKI_BINDER_BC_DECREFS:
					VG_(printf)("Binder Command Decrefs\n");
					break;
				case VKI_BINDER_BC_INCREFS_DONE:
					VG_(printf)("Binder Command Increfs Done\n");
					break;
				case VKI_BINDER_BC_ACQUIRE_DONE:
					VG_(printf)("Binder Command Acquire Done\n");
					break;
				case VKI_BINDER_BC_ATTEMPT_ACQUIRE:
					VG_(printf)("Binder Command Attempt Acquire\n");
					break;
				case VKI_BINDER_BC_REGISTER_LOOPER:
					VG_(printf)("Binder Command Register Looper\n");
					break;
				case VKI_BINDER_BC_ENTER_LOOPER:
					VG_(printf)("Binder Command Enter Looper\n");
					break;
				case VKI_BINDER_BC_EXIT_LOOPER:
					VG_(printf)("Binder Command Exit Looper\n");
					break;
				case VKI_BINDER_BC_DEAD_BINDER_DONE:
					VG_(printf)("Binder Command Dead Binder Done\n");
					break;
				case VKI_BINDER_BC_CLEAR_DEATH_NOTIFICATION:
					VG_(printf)("Binder Command Clear Death Notification \n");
					break;
				case VKI_BINDER_BC_REQUEST_DEATH_NOTIFICATION:
					VG_(printf)("Binder Command Request Death Notification\n");
					break;
				default:
					VG_(printf)("warning: unknown binder command!\n");
#endif
		...
		case VKI_BINDER_SET_IDLE_TIMEOUT:
			VG_(printf)("%d syscall: ioctl binder: SET_IDLE_TIMEOUT\n", tid);
			break;
		case VKI_BINDER_SET_MAX_THREADS:
			VG_(printf)("%d syscall: ioctl binder: SET_MAX_THREADS\n", tid);
			break;
		case VKI_BINDER_SET_IDLE_PRIORITY:
			VG_(printf)("%d syscall: ioctl binder: SET_IDLE_PRIORITY\n", tid);
			break;
		case VKI_BINDER_SET_CONTEXT_MGR:
			VG_(printf)("%d syscall: ioctl binder: SET_CONTEXT_MGR\n", tid);
			break;
		case VKI_BINDER_THREAD_EXIT:
			VG_(printf)("%d syscall: ioctl binder: THREAD_EXIT\n", tid);
			break;
		case VKI_BINDER_VERSION:
			VG_(printf)("%d syscall: ioctl binder: VERSION\n", tid);
			break;
#endif
		}
		break;
#ifdef VERBOSE_DEBUG
	default:
		VG_(printf)("%d syscall: syscallno=%d with %d args\n", tid, syscallno, nArgs);
#endif
	}
}
                            ...
                            UWord* args, UInt nArgs, SysRes res)
{
    ...
    case __NR_pread64:
      ...
      break;
    case __NR_ioctl:
      TNT_(syscall_ioctl)(tid, args, nArgs, res);
      break;
#endif
  }
}
